...
# 1) Copy all SQL Server tables into Neo4j as nodes
        ...
        # 'Process',
# -----------------------------------------------------------------------------
def cp_from_sqlserver_to_neo4j(sql_server_cursor, neo4j_session, url, username, password, last_data_pulled_date):
    tables = [
        'Application',
        'ApplicationVersion',
        'Component',
    ...
    #         query = f"MATCH (n:{tbl}) DETACH DELETE n"
    ...
    for table in tables:
        ...
        # fetch all rows
        select_sql = f"SELECT * FROM {table} WHERE ModifiedDt >= %s"
        sql_server_cursor.execute(select_sql, (last_data_pulled_date,))
...
def extract_tcode(narrative):
    ...
    return ""
# -------------------------------------------------------------------------------
# 2. New helper to upsert a single DataUpload node
# -------------------------------------------------------------------------------
def upsert_dataupload_node(neo4j_session):
    """
    Ensures a single DataUpload node exists and sets/updates
    its LastDataPulledDate to the current timestamp.
    """
    ts = datetime.now().isoformat()
    query = """
    MERGE (d:DataUpload {name: "DataUpload"})
    SET d.LastDataPulledDate = $ts
    """
    neo4j_session.run(query, ts=ts)

# -------------------------------------------------------------------------------
# New helper to fetch the last run date
# -------------------------------------------------------------------------------
def get_last_data_pulled_date(neo4j_session):
    """
    Returns the date portion (YYYY-MM-DD) of DataUpload.LastDataPulledDate,
    or a default if not set.
    """
    q = """
    MATCH (d:DataUpload {name: "DataUpload"})
    RETURN d.LastDataPulledDate AS last_date
    """
    rec = neo4j_session.run(q).single()
    if rec and rec["last_date"]:
        try:
            # parse ISO timestamp and return just the date
            dt = datetime.fromisoformat(rec["last_date"])
            return dt.strftime("%Y-%m-%d")
        except ValueError:
            # if it's not ISO, just return as‐is
            return rec["last_date"]
    # fallback if node or prop not found
    return " "
...
def main():
    ...
    with driver.session() as neo4j_sess:
        # 0) fetch the last time we pulled data
        last_date = get_last_data_pulled_date(neo4j_sess)
        print(f" LastDataPulledDate = {last_date}")

        # 1) bulk copy all raw tables changed since last pull
        cp_from_sqlserver_to_neo4j(
            sql_server_cursor,
            neo4j_sess,
            url, username, password,
            last_data_pulled_date=last_date
        )

        # 2) record this run in the DataUpload node (updates to now)
        upsert_dataupload_node(neo4j_sess)

    # Find TCodes and SubProcess
